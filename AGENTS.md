...
- Wait with `wait_for_review.ps1 -RunDir "<run-dir>" -MaxSeconds 60`.
...
If the bounded wait reports `STILL_WAITING`, send a brief progress update and wait again.
If the wait command is interrupted, times out, or exits unexpectedly, check `<run-dir>/status.txt`; consume the instruction when it is `INSTRUCTION_RECEIVED`, otherwise resume waiting. Do not treat an interrupted wait as completion.
